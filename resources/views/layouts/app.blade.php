...
{{-- NAVBAR --}}
<header class="bg-white/80 backdrop-blur border-b border-slate-200 dark:bg-slate-800/80 dark:border-slate-700">
    <div class="max-w-6xl mx-auto px-4 py-3 flex items-center justify-between gap-4">
        ...
        <div class="flex items-center gap-2">
            {{-- neskôr nahradíme auth logikou --}}
            <a href="{{ url('/login') }}" class="hidden sm:inline-block text-xs px-3 py-1 rounded border border-emerald-500 text-emerald-600 hover:bg-emerald-50">
                Prihlásenie
            </a>
            <a href="{{ url('/register') }}" class="hidden sm:inline-block text-xs px-3 py-1 rounded bg-emerald-500 text-white hover:bg-emerald-600">
                Registrácia
            </a>

            {{-- prepínač svetlý/tmavý režim --}}
            <button
                id="theme-toggle"
                type="button"
                class="inline-flex items-center justify-center h-9 w-9 rounded-full border border-slate-300 dark:border-slate-600 text-slate-700 dark:text-slate-100 text-sm"
            >
                ☀️
            </button>
        </div>
